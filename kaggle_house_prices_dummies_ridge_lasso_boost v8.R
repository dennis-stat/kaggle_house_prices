...
train_test_numeric <- train_test %>% 
     select_if(
         funs(is.numeric(.) & max(as.integer(.), na.rm = T)>10 & !is.factor(.))
         ) %>% select(-Id)
...
# Now lets run the lm
lasso.lm <- lm(SalePrice~., lm.exp.log.train.data)

#Now the final combined predictions
lasso.pred_full.train <- lasso.pred_log.train #  predict(lasso.lm, newdata = lm.exp.log.train.data)
lasso.pred_full.test <- lasso.pred_log.test # predict(lasso.lm, newdata = lm.exp.log.test.data)

mean((lasso.pred_full.train-train_df_exp$SalePrice)^2)
...
rf_test <- randomForest(resid ~ . ,data = train_df_exp %>% select(-SalePrice), importance = TRUE,  ntree=500)
...
predict_resid_train <- predict(rf,train_df_exp %>% select(-SalePrice)%>% mutate(
    lasso_pred = lasso.pred.train.full
) )
...
mean((predict_resid_train+lasso.pred.train.log-train_df_exp$SalePrice)^2)

# Now calculate test 
predict_resid_test <- predict(rf,test_df_exp %>% select(-SalePrice)%>% mutate(
    lasso_pred = lasso.pred.test.full
) )
