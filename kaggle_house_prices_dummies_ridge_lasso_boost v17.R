...
# Fix some NA's 
train_test <- train_test %>% 
    mutate(is_new_Garage = as.integer(abs(GarageYrBlt-YearBuilt)<2)) %>% 
...
Yr_Nb_slope <- Yr_Nb_slope %>% filter(term == 'YrSold')
Yr_Nb_slope_final  <- Yr_Nb_slope %>% select(Neighborhood, slope = estimate ) %>% ungroup() %>% 
    mutate(slope = slope -min(slope)+1)
