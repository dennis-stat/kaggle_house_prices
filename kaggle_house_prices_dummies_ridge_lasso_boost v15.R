...
train_test <- train_test %>% 
    mutate(is_Remod = as.integer(YearRemodAdd == YearBuilt),
           ...
           YearBuilt_bucket = factor(floor((YearBuilt_age/max(YearBuilt_age))/0.1)),
           YearRemodAdd_bucket = factor(floor((YearRemodAdd_age/max(YearRemodAdd_age))/0.1)),IsNew = ifelse(YrSold==YearBuilt, 1, 0)
...
train_test_numeric_skew <- train_test_numeric #%>% select_if(funs(abs(skew(.))>1.1 )) 
