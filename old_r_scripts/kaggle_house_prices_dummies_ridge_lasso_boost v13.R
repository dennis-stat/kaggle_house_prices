...
train_test_numeric_log <- train_test_numeric %>% 
    mutate_if(abs(skew(train_test[,i]))>1, log)
...
rf <- randomForest(resid ~ . ,data = train_test %>% filter(train_test == 1) %>% select(-SalePrice, -train_test) %>% mutate(resid = resid, lasso_pred = lasso.pred_log.train), importance = TRUE,  ntree=500)
...
predict_resid_train <- predict(rf,train_test %>% filter(train_test == 1) %>% select(-SalePrice, -train_test)%>% mutate( lasso_pred = lasso.pred_log.train) )
...
hist(delta)
# Now calculate test 
predict_resid_test <- predict(rf,train_test %>% filter(train_test == 0) %>% select(-SalePrice, -train_test)%>% mutate(lasso_pred = lasso.pred_log.test) )
