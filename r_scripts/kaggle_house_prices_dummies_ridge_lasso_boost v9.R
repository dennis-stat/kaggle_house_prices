...
rf <- randomForest(resid ~ . ,data = train_test %>% filter(train_test == 1) %>% select(-SalePrice, -train_test) %>% mutate(resid = resid), importance = TRUE,  ntree=500)
...
predict_resid_train <- predict(rf,train_test %>% filter(train_test == 1) %>% select(-SalePrice, -train_test) )
...
# Now calculate test 
predict_resid_test <- predict(rf,train_test %>% filter(train_test == 0) %>% select(-SalePrice, -train_test) )
