...
# Now lets run the lm
intercept <- 0
lasso.lm <- lm(I(SalePrice-intercept)~ 0+., lm.exp.log.train.data)

#Now the final combined predictions
lasso.pred_full.train <- predict(lasso.lm, newdata = lm.exp.log.train.data)
lasso.pred_full.test <- predict(lasso.lm, newdata = lm.exp.log.test.data)

mean((lasso.pred_full.train-train_df_exp$SalePrice)^2)

# calculate residuals 
resid <- train_df_exp$SalePrice-lasso.pred_full.train
...
predict_resid_train <- predict(rf,train_df_exp %>% select(-SalePrice))
...
mean((lasso.pred_full.train-train_df_exp$SalePrice)^2)
mean((predict_resid_train+lasso.pred_full.train-train_df_exp$SalePrice)^2)

# Now calculate test 
predict_resid_test <- predict(rf,test_df_exp %>% select(-SalePrice))


# Final predict 
predict <- lasso.pred_full.test+predict_resid_test
